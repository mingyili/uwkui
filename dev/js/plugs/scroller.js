...
(function($) {
    ...
    function Scroll(el, options) {
        ...
        if (this.options.role === 'slider') {
            ...
            this.scroller = $('.slider-content', this.wrapper)[0];
            $(this.scroller.children[0]).addClass('active');
            this.currentPage = 0;
            ...
            if (this.options.indicator) {
                var temp = '<ul class="slider-indicators">';
                ...
                this.indicator = $('.slider-indicators', this.wrapper)[0];
            }
        }
        // tab
        // ==================================
            ...
            this.options.scrollY = false;
            this.options.momentum = false;
            this.scroller = $('.tab-content', this.wrapper)[0];
            this.nav = $('.tabs-bar', this.wrapper)[0];
            $(this.scroller.children[0]).addClass('active');
            $(this.nav.children[0]).addClass('active');
            this.currentPage = 0;
            this.count = this.scroller.children.length;
            this.scroller.style.width = this.count + "00%";
            this.itemWidth = this.scroller.children[0].clientWidth;
    ...
    Scroll.prototype = {
        ...
        _end: function(e) {
            ...
            // tab
            // ==========================
            if (this.options.role === 'tab' && $(event.target).closest('ul').hasClass('tabs-bar')) {
            ...
            if (this.options.role === 'slider' || this.options.role === 'tab') {
                ...
                if (this.indicator && distanceX >= 30) {
                    $(this.indicator).children().removeClass('active');
                    $(this.indicator.children[this.currentPage]).addClass('active');
                } else if (this.nav && distanceX >= 30) {
                    $(this.nav).children().removeClass('active');
                    $(this.nav.children[this.currentPage]).addClass('active');
                }
                $(this.scroller).children().removeClass('active');
                $(this.scroller.children[this.currentPage]).addClass('active');
            }
        },
        _resize: function() {
            var that = this;
        ...
        _autoplay: function() {
            ...
            if (self.indicator) {
                $(self.indicator).children().removeClass('active');
                $(self.indicator.children[self.currentPage]).addClass('active');
                $(self.scroller).children().removeClass('active');
                $(self.scroller.children[self.currentPage]).addClass('active');
            } else if (self.nav) {
                $(self.nav).children().removeClass('active');
                $(self.nav.children[self.currentPage]).addClass('active');
                $(self.scroller).children().removeClass('active');
                $(self.scroller.children[self.currentPage]).addClass('active');
            }
    ...
    //数量实例化 $().scroller; 
    $.fn.scroller = function(option, v) {
        return this.each(function() {
            var $this = $(this),
                id = $this.data('scroller'),
                data = $.fn.scroller.scrollerData[id],
                options = typeof option == 'object' ? option: {};
            
            if (!data) {
                id = $.fn.scroller.scrollerData.index++;
                data = $.fn.scroller.scrollerData[id] = new Scroll(this, options);
                $this.data('scroller', id);
            };
            if (typeof option == 'string') data[option](v);
        });
    }
    $.fn.scroller.scrollerData = {index: 0};
    $.fn.scroller.Constructor = Scroll;
})($);