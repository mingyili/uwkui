...
;(function ($, w) {
    ...
    selectSku.prototype = {
        ...
        trigger: function() { 
            ...
            if (!this.opt.prdid) {$.toast('没有商品id'); return this;}
            if (!len || len === 1) {
                btn = !len ? btns : btns[0];
                btn.text = '确定'; 
        ...
        changePrice: function(oldprice, num) { 
            var data = prdData[this.prdid], price = parseFloat(oldprice);
            num = num || 1;
            //原价
            if(!isNaN(price) && (data.distype == "discount" || data.distype == "price")) skuDom.$old_price.html('￥' + price.toFixed(2)).show();
