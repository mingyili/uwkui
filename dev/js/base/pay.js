...
;(function ($, w) {
    ...
    Pay.prototype = {
        ...
        success: function() {
            ...
            var successUrl = this.opt.successUrl; 
            if(this.opt.success) this.opt.success.call(this);
            else setTimeout(function() { window.location.href = successUrl; }, 200);
        },
        end: function() {
            $.loadEnd();
        ...
        fail: function(info) {
            if(this.opt.fail) this.opt.fail.call(this, info);
                ...
                $.loadStart(), window.location.reload();
            });
            return this.end();
        },
        ...
        pay: function(payType, tid) {
            var pay = this.opt[(payType || 'wx') + 'Pay'];
            this.opt.tid = tid || this.opt.tid; //更换支付订单
            if(pay) {
